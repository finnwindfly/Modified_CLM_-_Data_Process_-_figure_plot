...
def load_and_process_temp_data(nc_path, time_slice, shallow_slice, deep_slice):
        nc_data = xr.open_dataset(nc_path)
        temp_shallow = nc_data['TSOI'].sel(time=time_slice).isel(levgrnd=shallow_slice) - 273.15
        temp_shallow_mean = temp_shallow.mean(dim='time').mean(dim='levgrnd')

        temp_deep = nc_data['TSOI'].sel(time=time_slice).isel(levgrnd=deep_slice) - 273.15
        temp_deep_mean = temp_deep.mean(dim='time').mean(dim='levgrnd')

        temp_min = nc_data['TSOI'].sel(time=time_slice).isel(levgrnd=slice(0, 12)) - 273.15
        temp_min_val = temp_min.mean(dim='levgrnd').min(dim='time')
...
def plot_map_set(ax, lon, lat, data, levels, title, shapefile_path, cb_ticks, cmap, clip_shape=False, clip_args=None):
        ...
        cb.ax.text(1.05, -0.88, '°C', fontsize=18, ha='left', va='center', transform=cb.ax.transAxes)
...
temp_original_shallow, temp_original_deep, temp_original_min = load_and_process_temp_data(original_path, ft_slice_time, shallow_slice, deep_slice)
temp_modified_shallow, temp_modified_deep, temp_modified_min = load_and_process_temp_data(modified_path, ft_slice_time, shallow_slice, deep_slice)
...
plot_args = [
        (gs[0, 0], temp_original_shallow, np.arange(-20, 20, 2), "(a)", np.arange(-20, 20, 5)),
        (gs[0, 1], temp_modified_shallow, np.arange(-20, 20, 2), "(b)", np.arange(-20, 20, 5)),
        (gs[0, 2], temp_df_shallow, np.arange(-0.3, 0.3, 0.1), "(c)", np.arange(-0.3, 0.3, 0.1)),
        (gs[1, 0], temp_original_deep, np.arange(-20, 20, 2), "(d)", np.arange(-20, 20, 5)),
        (gs[1, 1], temp_modified_deep, np.arange(-20, 20, 2), "(e)", np.arange(-20, 20, 5)),
        (gs[1, 2], temp_df_deep, np.arange(-0.35, 0.35, 0.1), "(f)", np.arange(-0.35, 0.35, 0.2)),
        (gs[2, 0], temp_original_min, np.arange(-20, 20, 2), "(g)", np.arange(-20, 20, 5)),
        (gs[2, 1], temp_modified_min, np.arange(-20, 20, 2), "(h)", np.arange(-20, 20, 5)),
        (gs[2, 2], temp_df_min, np.arange(-0.3, 0.3, 0.1), "(i)", np.arange(-0.3, 0.3, 0.1)),
]
...
plt.savefig(
        fname='/Users/finn/Library/CloudStorage/OneDrive-Personal/学习工作相关/博士生工作/博士工作相关/科研工作/Development_and_Validation_of_a_New_Soil_frost_heave_shceme/fig/after20241223/fig15_new-Comparative analysis of soil temperature simulations on the Qinghai-Tibet Plateau between the new and original schemes.pdf',
