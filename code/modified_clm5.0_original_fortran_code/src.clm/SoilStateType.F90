...
!scs: vangenuchten
     ...
     real(r8), pointer :: dyn_watsat           (:,:) ! dynamic volumetric soil waterat saturation (porosity)
     real(r8), pointer :: dyn_bd               (:,:) ! col bulk density of dry soil material [kg/m^3] (CN)
     real(r8), pointer :: dyn_dz               (:,:)
     real(r8), pointer :: ori_dz               (:,:)
     real(r8), pointer :: dyn_z                (:,:)
     real(r8), pointer :: dyn_zi               (:,:)
     real(r8), pointer :: dyn_ctriwat          (:,:)
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    allocate(this%dyn_watsat           (begc:endc,nlevgrnd))            ; this%dyn_watsat           (:,:) = nan
    allocate(this%dyn_bd               (begc:endc,nlevgrnd))            ; this%dyn_bd               (:,:) = nan
    allocate(this%dyn_dz               (begc:endc,nlevgrnd))            ; this%dyn_dz               (:,:) = nan
    allocate(this%ori_dz               (begc:endc,nlevgrnd))            ; this%ori_dz               (:,:) = nan
    allocate(this%dyn_z                (begc:endc,nlevgrnd))            ; this%dyn_z                (:,:) = nan
    allocate(this%dyn_zi               (begc:endc,0:nlevgrnd))          ; this%dyn_zi               (:,:) = nan
    allocate(this%dyn_ctriwat          (begc:endc,nlevgrnd))            ; this%dyn_ctriwat          (:,:) = nan
  ...
  subroutine InitHistory(this, bounds)
    ...
    ! Finn add for put out dyn_watsat and dyn_bd
    this%dyn_watsat(begc:endc,:) = spval
       call hist_addfld2d (fname='DYNPOROSITY', units='m^3/m^3', type2d='levgrnd', &
            avgflag='A', long_name='dynamic soil porosity(water saturated)', &
            ptr_col=this%dyn_watsat)

    this%dyn_bd(begc:endc,:) = spval
       call hist_addfld2d (fname='DYNBD', units='kg/m^3', type2d='levgrnd', &
            avgflag='A', long_name='dynamic soil bulk density', &
            ptr_col=this%dyn_bd)

    data2dptr => this%dyn_dz(begc:endc,1:nlevsoi)
    call hist_addfld2d (fname='DYNDZ',  units='m', type2d='levsoi', &
         avgflag='A', long_name='dynamic soil thickness(vegetated landunits only)',&
         ptr_col=data2dptr, l2g_scale_type='veg')

    data2dptr => this%dyn_z(begc:endc,1:nlevsoi)
    call hist_addfld2d (fname='DYNZ',  units='m', type2d='levsoi', &
         avgflag='A', long_name='dynamic soil layer depth(vegetated landunits only)',&
         ptr_col=data2dptr, l2g_scale_type='veg')

    data2dptr => this%dyn_zi(begc:endc,1:nlevsoi)
    call hist_addfld2d (fname='DYNZI',  units='m', type2d='levsoi', &
         avgflag='A', long_name='dynamic soil interface depth(vegetated landunits only)',&
         ptr_col=data2dptr, l2g_scale_type='veg')

    this%dyn_ctriwat(begc:endc,:) = spval
    call hist_addfld2d (fname='DYNCTRIWAT',  units='m', type2d='levgrnd', &
         avgflag='A', long_name='dynamic soil frost ctriwat(vegetated landunits only)',&
        ptr_col=this%dyn_ctriwat, l2g_scale_type='veg')

    !------modified by Finn for output sand--------------
    this%cellsand_col(begc:endc,:) = spval
    call hist_addfld2d (fname='SAND_COL', units='%', type2d='levsoi', &
         avgflag='A', long_name='soil precent sand', &
         ptr_col=this%cellsand_col, l2g_scale_type='veg')
    this%cellclay_col(begc:endc,:) = spval
    call hist_addfld2d (fname='CLAY_COL', units='%', type2d='levsoi', &
         avgflag='A', long_name='soil precent clay', &
         ptr_col=this%cellclay_col, l2g_scale_type='veg')
    this%cellorg_col(begc:endc,:) = spval
    call hist_addfld2d (fname='ORGANIC_COL', units='kg/m3', type2d='levsoi',&
         avgflag='A', long_name='soil organic matter', &
         ptr_col=this%cellorg_col, l2g_scale_type='veg')
    this%bd_col(begc:endc,:) = spval
    call hist_addfld2d (fname='BD_COL', units='kg/m3', type2d='levgrnd', &
         avgflag='A', long_name='soil buld density', &
         ptr_col=this%bd_col, l2g_scale_type='veg')
    this%hksat_col(begc:endc,1:nlevsoi) = spval
    call hist_addfld2d (fname='HKSAT_COL', units='mm/s', type2d='levsoi', &
         avgflag='A', long_name='soil saturated hydraulic conductivity', &
         ptr_col=this%hksat_col, l2g_scale_type='veg')
    this%tksatu_col(begc:endc,1:nlevsoi) = spval
    call hist_addfld2d (fname='TKSAT_COL', units='W/m-K', type2d='levsoi', &
         avgflag='A', long_name='soil saturated thermal conductivity', &
         ptr_col=this%tksatu_col, l2g_scale_type='veg')
  ...
  subroutine InitCold(this, bounds)
    ...
    !-----------------------------------------------------------
    this%dyn_ctriwat(bounds%begc:bounds%endc,1:nlevgrnd) = 0._r8
    do c = bounds%begc, bounds%endc
       this%dyn_zi(c,0) = 0._r8
       do j = 1, nlevgrnd
          this%ori_dz(c,j) = col%dz(c,j)
          this%dyn_dz(c,j) = col%dz(c,j)
          !this%dyn_watsat(c,j) = 0.48_r8
          this%dyn_zi(c,j) = col%zi(c,j)
          this%dyn_z(c,j)  = col%z(c,j)
       end do
    end do
