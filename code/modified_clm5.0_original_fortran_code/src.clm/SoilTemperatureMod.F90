...
contains
  ...
  subroutine SoilThermProp (bounds,  num_nolakec, filter_nolakec, &
    ...
    real(r8) :: zh2osfc
    ! Finn's modification for frost heave 
    real(r8) :: watsat0
    real(r8) :: bd0
    real(r8) :: ctriwat
    real(r8) :: om_frac
    ! Finn's modificaton ends here
    !-----------------------------------------------------------------------
         ...
         do fc = 1, num_nolakec
            ...
            if (j >= 1) then    
               ...
               if ((col%itype(c) == icol_sunwall .OR. col%itype(c) == icol_shadewall) .and. j <= nlevurb) then
                  ...
                  ! Finn's modificaiton
                  satw = (h2osoi_liq(c,j)/denh2o + h2osoi_ice(c,j)/denice)/(soilstate_inst%dyn_dz(c,j)*soilstate_inst%dyn_watsat(c,j))
                  ! Finn's modificaiton ends here
                  satw = min(1._r8, satw)
                  ...
                  ! Finn's modification starts 
                     fl = (h2osoi_liq(c,j)/(denh2o*soilstate_inst%dyn_dz(c,j))) / (h2osoi_liq(c,j)/(denh2o*soilstate_inst%dyn_dz(c,j)) + &
                          h2osoi_ice(c,j)/(denice*soilstate_inst%dyn_dz(c,j)))
                     dksat = tkmg(c,j)*tkwat**(fl*soilstate_inst%dyn_watsat(c,j))*tkice**((1._r8-fl)*soilstate_inst%dyn_watsat(c,j))
             ! Finn's modification ends here
      ...
      do j = 1, nlevgrnd
         do fc = 1,num_nolakec
            ...
            if ((col%itype(c) == icol_sunwall .OR. col%itype(c) == icol_shadewall) .and. j <= nlevurb) then
               ...
               ! Finn's modificaiton starts
               cv(c,j) = csol(c,j)*(1-soilstate_inst%dyn_watsat(c,j))*soilstate_inst%dyn_dz(c,j) + (h2osoi_ice(c,j)*cpice + h2osoi_liq(c,j)*cpliq)
         ! Finn's modification ends here
  ...
  subroutine Phasechange_beta (bounds, num_nolakec, filter_nolakec, dhsdT, &
    ...
    real(r8) :: smp                                !frozen water potential (mm)
    ! Finn's modification for frost heave 
    real(r8) :: watsat0(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: bd0(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: ctriwat
    real(r8) :: vliqice0(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: vliqice(bounds%begc:bounds%endc, 1:nlevgrnd)
    real(r8) :: ctriice(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: ctridz(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: dyn_dz0(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: dyn_watsat0(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: icechange(bounds%begc:bounds%endc,1:nlevgrnd)
    real(r8) :: varphi0
    ! Finn's update hydrothermal properties:
    ...
    associate(                                                        & 
      ...
      do j = 1, nlevgrnd
         do fc = 1, num_nolakec
            c = filter_nolakec(fc)
               vliqice0(c,j) = (h2osoi_liq(c,j)/denh2o+h2osoi_ice(c,j)/denice)/soilstate_inst%dyn_dz(c,j)
               watsat0(c,j)  = soilstate_inst%dyn_watsat(c,j)
               bd0(c,j)      = soilstate_inst%dyn_bd(c,j)
         end do
      enddo
      ! Finn's modification ends here
            ...
            if ((col%itype(c) /= icol_sunwall .and. col%itype(c) /= icol_shadewall &
               ...
               if (lun%itype(l) == istsoil .or. lun%itype(l) == istcrop .or. col%itype(c) == icol_road_perv) then
                  if(t_soisno(c,j) < tfrz) then
                     ...
                     ! Finn's modification 
                     supercool(c,j) = soilstate_inst%dyn_watsat(c,j)*(smp/sucsat(c,j))**(-1._r8/bsw(c,j))
                     supercool(c,j) = supercool(c,j)*soilstate_inst%dyn_dz(c,j)*1000._r8
             ! Finn's modification ends here
                  endif
               endif
      ...
      ! Finn's update for dyn_watsat & dyn_dz
      do j = 1,nlevgrnd
         do fc = 1,num_nolakec
            c = filter_nolakec(fc)
              ctriwat = 0._r8
              dyn_watsat0(c,j) = soilstate_inst%dyn_watsat(c,j)
              dyn_dz0(c,j) = soilstate_inst%dyn_dz(c,j)
              if ((h2osoi_ice(c,j) > 0._r8) .AND. (vliqice0(c,j) > (0.8_r8*watsat0(c,j)))) then
                 icechange(c,j)  = h2osoi_ice(c,j) - wice0(c,j)
                 ctriice(c,j)  = (h2osoi_ice(c,j)/(denice*soilstate_inst%dyn_dz(c,j)))-(h2osoi_ice(c,j)/(denh2o*soilstate_inst%dyn_dz(c,j)))
                 ctriice(c,j)  = min(ctriice(c,j),h2osoi_ice(c,j)/(denice*soilstate_inst%dyn_dz(c,j))*0.5_r8 )
                 ctridz(c,j)   = ((h2osoi_ice(c,j)/(denice*soilstate_inst%dyn_dz(c,j)))-(h2osoi_ice(c,j)/(denh2o*soilstate_inst%dyn_dz(c,j))))
                 ctridz(c,j)   = min(ctridz(c,j),h2osoi_ice(c,j)/(denice*soilstate_inst%dyn_dz(c,j))*0.5_r8 )
                 varphi0 = 0.01_r8*(soilstate_inst%cellsand_col(c,j)**2._r8)+0.02_r8*(soilstate_inst%cellclay_col(c,j)**2._r8)+0.03_r8*(soilstate_inst%cellorg_col(c,j)**2._r8)
                 varphi0 = varphi0 / (0.04*soilstate_inst%cellsand_col(c,j)+0.05*soilstate_inst%cellclay_col(c,j)+0.06*soilstate_inst%cellorg_col(c,j))
                 !varphi0 = varphi0 * 1.5_r8
                 if (icechange(c,j) > 0._r8) then
                    soilstate_inst%dyn_watsat(c,j) = watsat(c,j) + ctriice(c,j)
                    soilstate_inst%dyn_watsat(c,j) = min(watsat(c,j)*varphi0,soilstate_inst%dyn_watsat(c,j))
                    soilstate_inst%dyn_dz(c,j) = soilstate_inst%ori_dz(c,j) * (1._r8 + ctridz(c,j))
                    soilstate_inst%dyn_dz(c,j) = min(soilstate_inst%dyn_dz(c,j),soilstate_inst%ori_dz(c,j)*varphi0)
                 else if (icechange(c,j) == 0._r8) then
                    !ctriice(c,j) = wmass0(c,j) - supercool(c,j)
                    ctriice(c,j) = (h2osoi_ice(c,j)/(denice*soilstate_inst%dyn_dz(c,j))) - &
                                   (h2osoi_ice(c,j)/(denh2o*soilstate_inst%dyn_dz(c,j)))
                    ctriice(c,j)  = ctriice(c,j)!*1.5_r8!(0.123_r8 * soilstate_inst%cellclay_col(c,j))
                    soilstate_inst%dyn_watsat(c,j) = watsat(c,j) + ctriice(c,j)
                    soilstate_inst%dyn_watsat(c,j) = min(0.8_r8,soilstate_inst%dyn_watsat(c,j))
                    soilstate_inst%dyn_dz(c,j) = dyn_dz0(c,j)
                    !soilstate_inst%dyn_dz(c,j) =
                    !min(soilstate_inst%dyn_dz(c,j),col%dz(c,j)*1.2_r8)
                 else if (icechange(c,j) < 0._r8) then
                    if (imelt(c,j) == 2) then
                       soilstate_inst%dyn_watsat(c,j) = dyn_watsat0(c,j)
                       soilstate_inst%dyn_dz(c,j)     = dyn_dz0(c,j)
                       !soilstate_inst%dyn_dz(c,j) = soilstate_inst%dyn_dz(c,j)
                       !- (soilstate_inst%dyn_dz(c,j) - col%dz(c,j))*0.5_r8
                       !soilstate_inst%dyn_dz(c,j) =
                       !max(col%dz(c,j),soilstate_inst%dyn_dz(c,j))
                    else
                       !ctriice(c,j) =
                       !(icechange(c,j)/(denice*soilstate_inst%dyn_dz(c,j)))-(icechange(c,j)/(denh2o*soilstate_inst%dyn_dz(c,j)))
                       soilstate_inst%dyn_watsat(c,j) = watsat(c,j) + ctriice(c,j)
                       soilstate_inst%dyn_watsat(c,j) = max(watsat(c,j), soilstate_inst%dyn_watsat(c,j))
                       if (soilstate_inst%dyn_dz(c,j) > soilstate_inst%ori_dz(c,j)) then
                          soilstate_inst%dyn_dz(c,j) = soilstate_inst%dyn_dz(c,j) - (soilstate_inst%dyn_dz(c,j) - soilstate_inst%ori_dz(c,j))*0.5_r8
                          soilstate_inst%dyn_dz(c,j) = max(soilstate_inst%ori_dz(c,j), soilstate_inst%dyn_dz(c,j))
                       else 
                          soilstate_inst%dyn_dz(c,j) = col%dz(c,j)
                       endif
                    endif
                 endif
              else
                 soilstate_inst%dyn_watsat(c,j) = watsat(c,j)
                 soilstate_inst%dyn_dz(c,j)     = col%dz(c,j)
              endif
              soilstate_inst%dyn_ctriwat(c,j)  = ctriice(c,j)
              col%dz(c,j)                      = soilstate_inst%dyn_dz(c,j)
              col%zi(c,j)                      = sum(soilstate_inst%dyn_dz(c,1:j))
              col%z(c,j)                       = 0.5 * (col%zi(c,j-1) + col%zi(c,j))
              soilstate_inst%dyn_zi(c,j)       = col%zi(c,j)
              soilstate_inst%dyn_z(c,j)        = col%z(c,j)
              !soilstate_inst%bd_col(c,j)       =
              !(dyn_dz0(c,j)/soilstate_inst%dyn_dz(c,j))*soilstate_inst%bd_col(c,j)
              soilstate_inst%dyn_bd(c,j)       = (dyn_dz0(c,j)/soilstate_inst%dyn_dz(c,j))*soilstate_inst%bd_col(c,j)
              !soilstate_inst%cellsand_col(c,j) =
              !(soilstate_inst%ori_dz(c,j)/soilstate_inst%dyn_dz(c,j))*soilstate_inst%cellsand_col(c,j)
              !soilstate_inst%cellclay_col(c,j) =
              !(soilstate_inst%ori_dz(c,j)/soilstate_inst%dyn_dz(c,j))*soilstate_inst%cellclay_col(c,j)
              !soilstate_inst%cellorg_col(c,j)  =
              !(soilstate_inst%ori_dz(c,j)/soilstate_inst%dyn_dz(c,j))*soilstate_inst%cellorg_col(c,j)
              
              !call ReadNL( nlfilename )
              !call SoilStateInitTimeConst(bounds, soilstate_inst,
              !nlfilename_loc)
              !om_frac = soilstate_inst%cellorg_col(c,j) / 130.0_r8
              !om_b = 2.7_r8  + (9.3_r8*(soilstate_inst%dyn_z(c,j)/0.5_r8))
              !om_b = min(om_b, 12._r8)
              !om_b              = min(2.7_r8  + 9.3_r8
              !*(soilstate_inst%dyn_z(c,j)/0.5_r8), 12.0_r8)
              !om_sucsat         = min(10.3_r8 - 0.2_r8
              !*(soilstate_inst%dyn_z(c,j)/0.5_r8), 10.1_r8)
              !soilstate_inst%bsw_his(c,j)         = (1._r8-om_frac) * (2.91_r8
              !+0.159_r8*soilstate_inst%cellclay_col(c,j)) + om_frac*om_b
              !soilstate_inst%sucsat_his(c,j)      = 10._r8 * (
              !10._r8**(1.88_r8-0.0131_r8*soilstate_inst%cellsand_col(c,j)))
              !soilstate_inst%sucsat_his(c,j)    = (1._r8-om_frac) *
              !soilstate_inst%sucsat_his(c,j) + om_sucsat*om_frac
              tkm                            = (1._r8-om_frac) * (8.80_r8*soilstate_inst%cellsand_col(c,j)+2.92_r8*soilstate_inst%cellclay_col(c,j))&
                                               /(soilstate_inst%cellsand_col(c,j)+soilstate_inst%cellclay_col(c,j))+ 0.25_r8*om_frac ! W/(m K)
              soilstate_inst%tkmg_col(c,j)   = tkm ** (1._r8-soilstate_inst%dyn_watsat(c,j)) 
              soilstate_inst%tkdry_col(c,j)  = ((0.135_r8*soilstate_inst%dyn_bd(c,j) + 64.7_r8) / &
                     (2.7e3_r8 - 0.947_r8*soilstate_inst%dyn_bd(c,j)))*(1._r8-om_frac) + 0.05_r8*om_frac
              !xksat         = 0.0070556
              !*(10.**(-0.884+0.0153*soilstate_inst%cellsand_col(c,j)) )
              !om_hksat          = max(0.28_r8 -
              !0.2799_r8*(col%z(c,j)/0.5_r8),xksat) 
              !soilstate_inst%hksat_min_col(c,j) = xksat
              ...
              !   uncon_hksat = 0._r8
              !end if
              !soilstate_inst%hksat_his(c,j)  = uncon_frac*uncon_hksat +
