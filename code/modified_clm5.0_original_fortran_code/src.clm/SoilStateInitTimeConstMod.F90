       ...
       if (lun%itype(l)==istwet .or. lun%itype(l)==istice_mec .or. &
          ...
          do lev = 1,nlevgrnd
             ...
             if (lun%itype(l) /= istdlak) then  ! soil columns of both urban and non-urban types
                ...
                ! Finn's add for dyn_watsat and dyn_bd 
                soilstate_inst%dyn_watsat(c,lev)    = soilstate_inst%watsat_col(c,lev)
                soilstate_inst%dyn_bd(c,lev)        = (1._r8 - soilstate_inst%dyn_watsat(c,lev))*2.7e3_r8
                ! ends here
