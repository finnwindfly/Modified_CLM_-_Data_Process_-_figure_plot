      ...
      do fc = 1, num_hydrologyc
         ...
         do ! continuous do loop with exit clause
            ...
            do j = 1, nlayers
               ...
               !vwc_liq(c,j) = max(h2osoi_liq(c,j),1.0e-6_r8)/(dz(c,j)*denh2o)
            !Finn's modification
               vwc_liq(c,j) = max(h2osoi_liq(c,j),1.0e-6_r8)/(soilstate_inst%dyn_dz(c,j)*denh2o)
...
!     using array sections allowed consistent 1d indexing throughout
    associate(&
         ...
         do j = 1, nlayers
            ...
            ! Finn's modification
            s2(j) = vwc_liq(j)/soilstate_inst%dyn_watsat(c,j)
  ...
  ! impose constraints on relative saturation at the layer interface
            ...
            !Finn 's modification           
            dsmpdw(j) = dsmpds /  soilstate_inst%dyn_watsat(c,j)
         ! Finn's modification ends here  
